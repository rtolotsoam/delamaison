  ...
  class Datatables
  {
      ...
      if($sSearch != ''){  
        for($i = 0; $i < count($mColArray); $i++){
          if($this->ci->input->post('bSearchable_' . $i) == 'true' && in_array($mColArray[$i], $columns))
            $sWhere .= $this->select[$mColArray[$i]] . "::text ILIKE '%" . $sSearch . "%' OR ";
		}
     }
        ...
        if(isset($_POST['sSearch_' . $i]) && $this->ci->input->post('sSearch_' . $i) != '' && in_array($mColArray[$i], $columns))
        {
          ...
          foreach($miSearch as $val)
          {
            if(preg_match("/(<=|>=|=|<|>)(\s*)(.+)/i", trim($val), $matches))
              ...
              $this->ci->db->where($this->select[$mColArray[$i]].'::text ILIKE', '%'.$val.'%');
          }
        }
      }
    ...
    */
    protected function produce_output($charset)
    {
      ...
      foreach ($aaData as $val_dat) {
        $data = array();          
        foreach ($val_dat as $val_tab) {
              array_push($data, ascii_to_entities($val_tab));
        }

        array_push($bbData, $data);
      }
